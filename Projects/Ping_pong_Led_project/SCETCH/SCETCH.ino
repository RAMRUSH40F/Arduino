...
#define LED_NUM 96   // кол-во светодиодо
//97
#define LED_BR 100     // яркость ленты

#define B1_PIN 4      // пин кнопки 1
#define B2_PIN 6      // пин кнопки 2
#define BUZZ_PIN 5    // пин пищалки

#define ZONE_SIZE 10  // размер зоны
#define MIN_SPEED 9   // минимальная скорость
#define MAX_SPEED 14  // максимальная скорость
#define WIN_SCORE 3   // победный счёт
...
void gameRoutine() {
  if (millis() - tmr >= 10) {   // каждые 10 мс
    ...
    FastLED.clear();
    fillZones(CRGB::Green, CRGB:: Pink);  // показываем зоны
    leds[pos / 10] = CRGB:: Blue  ;        // рисуем мячик
    FastLED.show();
  }
}
